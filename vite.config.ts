...
export default defineConfig({
  ...
  server: {
    ...
    fs: {
      ...
      // собирается, — её собственный каталог, зависимости и два общих с
      // демоном модуля (`routes.ts` — разбор адресов, `grouping.ts` — склейка
      // прогонов с пайплайнами). Целый корень репозитория здесь означал бы,
