...
import { row as run } from './commands/run.js';
import { row as resume } from './commands/resume.js';
import { row as diff } from './commands/diff.js';
import { row as decide } from './commands/decide.js';
import { row as propose } from './commands/propose.js';
import { row as widgets } from './commands/widgets.js';
import { row as apply } from './commands/apply.js';
import { row as lint } from './commands/lint.js';
import { row as status } from './commands/status.js';
import { row as logs } from './commands/logs.js';
import { row as config } from './commands/config.js';
import { row as schema } from './commands/schema.js';
import { row as plugins } from './commands/plugins.js';
import { row as gc } from './commands/gc.js';
import { row as init } from './commands/init.js';
import { row as context } from './commands/context.js';
import { row as up } from './commands/up.js';
import { row as down } from './commands/down.js';
import { row as usage } from './commands/usage.js';
import { row as backlog } from './commands/backlog.js';
import { row as knowledge } from './commands/knowledge.js';
import { row as data } from './commands/data.js';
import { row as mergeLanes } from './commands/merge-lanes.js';
import { row as assertClean } from './commands/assert-clean.js';
import { row as project } from './commands/project.js';
 ...
 * (помощник объявления — `src/cli/commandRow.ts`, отдельным модулем: если бы
 * он жил здесь, перечень и каждый из 25 модулей команд образовали бы цикл
 * импорта по значению). Точка входа (`src/cli/main.ts`) подаёт его
 * `resolveConfig` (`COMMAND_ROW_IDS`) и `loadPlugins` (этот перечень) тем же
 * механизмом, каким `stepcast up` подаёт `src/ui/rows.ts`: дерево команд CLI =
 ...
 * строка объявляет `inject: PIPELINE_SERVICES` (`src/cli/commandRow.ts`),
...
/** Id строк команд — то, чем `core/config/resolve.ts` заводит семя дерева наравне с `BUILTIN_ROW_IDS`. */
