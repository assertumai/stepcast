 ...
 * работает как прежде.
 */
export { definePlugin } from '../kernel/define.js';
 ...
 */
export { parseDuration } from '../kernel/units.js';

/** Строка дерева плагинов: то, что команда получает в `CommandEnv.pluginTree`. */
export type { TreeRow, TreeRowSource } from '../kernel/tree/tree.js';
 ...
 */
export type { Context, ContributionRegistrar, Inject } from '../kernel/context.js';

export type { CliIo, CommandSpec, FlagKind, FlagSpec, ParsedArgs } from '../kernel/cli/types.js';

/** Запуск дочернего процесса под надзором: то же, чем движок исполняет шаги. */
export { runProcess, DEFAULT_GRACE_MS } from '../parts/pipeline/run/exec/process.js';
export type { ProcessOptions, ProcessResult, ProcessOutcome } from '../parts/pipeline/run/exec/process.js';

export { ExitCode, StepcastError, isStepcastError } from '../kernel/errors.js';
export type { ExitCodeValue } from '../kernel/errors.js';
