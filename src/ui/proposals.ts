...
import type { ProposalRecord } from '../core/proposals/entry.js';
import { proposalsDirPath, readProposalsDir, type ProposalsReadResult } from '../core/proposals/store.js';

/**
 * Обзор очереди предложений всех проектов и отпечаток её каталога — тем же
 * устройством, что `buildWidgets`/`widgetFingerprint` (`src/ui/widgets.ts`) и
 * `buildDashboards`/`dashboardsDirFingerprint` (`src/ui/dashboardsFile.ts`):
 * этот модуль обходит `projects.json`, чтение одного проекта — забота
 * `core/proposals/store.ts`.
 */
