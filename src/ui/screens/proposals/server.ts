...
import { acceptProposal, rejectProposal, resolveProposalTarget } from '../../../core/proposals/store.js';
 ...
 * содержимым цели — диф строит браузер (`src/core/textDiff.ts`); `POST
...
const handlePost: ApiHandler = async (req, res, env) => {
  ...
  try {
    ...
    // что сделал сам (тот же приём, что у `src/ui/screens/routes/server.ts`).
    env.watcher.poll();
