...
import type { TreeRowSource } from './tree/tree.js';
 ...
 * осмотру называется причиной»). Заполняется вызывающим демона (`src/parts/ui/screens/plugins/server.ts`);
 ...
 * (`src/parts/ui/screens/plugins/server.ts`), у которого есть последний отчёт
