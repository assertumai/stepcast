 ...
 * символическую ссылку наружу (`src/parts/ui/widgets.ts`, `resolveWidgetFile`).
 ...
 * (`src/parts/pipeline/document/steps.ts`). Вызывающий (`applyTreeRow`,
 * `src/kernel/load.ts`) решает, прекращает ли этот отказ загрузку
