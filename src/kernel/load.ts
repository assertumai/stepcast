...
import { StepcastPluginSchema, type DeclarativeContributionFields, type PipelinePlugin } from '../parts/pipeline/contract.js';
import { translateReservedNameConflict, unresolvedFibers, type Context, type Fiber, type Kernel } from './kernel.js';
import { readPluginManifest, type PluginManifest } from './tree/manifest.js';
...
import { BUILTIN_USE_PREFIX, isBuiltinUse, type TreeRow } from './tree/tree.js';
 ...
 * Строка поставки витрины (`src/parts/ui/screens/registry.ts`, `screenRow()`)
 ...
 * витрины (`screenRow`, `src/parts/ui/screens/registry.ts`), строк движка (`partRow`,
 * `src/parts/pipeline/services.ts`) и строк команд (`src/parts/cli/rows.ts`): копии
...
/**
 * Узкий вход обхода дерева (`source-tree-microkernel-layout`, design.md,
 * Решение 5): ядро читает от конфигурации движка единственное поле — дерево
 * плагинов, а не импортирует `ResolvedConfig` целиком. Тип структурный:
 * `ResolvedConfig` подходит под него без единой правки — ни сигнатура наружу,
 * ни вызывающие не меняются.
 */
export interface PluginTreeSource {
 ...
 */
export async function applyPluginTree(kernel: Kernel, resolved: PluginTreeSource, options: LoadOptions): Promise<LoadResult> {
