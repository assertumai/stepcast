 ...
 * завело до первой строки (`createKernelShell`: встроенные команды CLI, виды
 * шага `agent`/`run`/`script`/`uses`, четыре служебных сервиса) и всякая
 * поздняя регистрация на корне. Такой вклад не приписывается строке наугад и не
