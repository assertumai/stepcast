 ...
 * знает файл работы, не конфигурация). Список, а не строка с разделителем:
 * разделитель не даёт объявить имя с пробелом (`npm run`, `./gradlew`).
 *
 * Пустой список отклоняется здесь же: объявление, не называющее ни одного
 * инструмента, почти наверняка забытая правка, а не «инструментов нет» — для
 * второго ключ просто не заводят. Каждый элемент — та же непустая строка, что
 * `check` и `spec.tool` (`CheckCommandSchema` переиспользуется, а не
 * копируется); пробел внутри значения намеренно допущен теми же причинами.
 */
