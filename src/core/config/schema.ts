...
export const RawProjectSchema = z
  .object({
    ...
    // который сам является рабочим деревом git. Модель — та же
    // `RelativeRepoPathSchema`, что у `spec.dir` и `edit_paths`, но с
    // дополнительным отказом символам шаблона глоба (`*`, `?`): значение
    // здесь — конкретный каталог, а не образец, по нему заводится
    // отдельное частное состояние составного якоря и им же префиксуются
    // пути в `changedPaths` и `diff.patch` — шаблон нечем ни открыть, ни
    // приписать к пути. Пустой список отклоняется той же причиной, что
    // `tools: []` и `edit_paths: []`.
    //
