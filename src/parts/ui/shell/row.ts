...
import { ApiService, ScreensService, screenRow, type ApiHandler, type RequestEnv } from '../screens/registry.js';
import { routesPayload } from '../routesFile.js';
import type { ActiveScreen } from '../screens/registry.js';
...
function handleEvents(req: IncomingMessage, res: ServerResponse, env: RequestEnv): void {
  ...
  // `src/parts/ui/daemon/server.ts`), и ссылка у него всегда новая.
  ...
  // отпечатка (`src/parts/ui/daemon/watcher.ts`), но новое значение — новый объект даже
  ...
  // наблюдателем только по сдвигу своей части отпечатка (`src/parts/ui/daemon/watcher.ts`),
  ...
  // каждый такт заново (`activeScreens`, `src/parts/ui/daemon/server.ts`).
 ...
 *
 * Имя экспорта — `row`, как у всякой строки встроенного слоя: перечень
 * (`src/parts/ui/rows.ts`) переименовывает её на импорте там, где нужно говорить о
 * ней по имени.
 */
export const row: BuiltinRow = screenRow('ui-shell', [], (ctx) => {
