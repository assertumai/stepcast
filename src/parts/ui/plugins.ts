...
import { pluginsDirPath } from '../../kernel/tree/discover.js';
import { readPluginManifest } from '../../kernel/tree/manifest.js';
 ...
 * относительные импорты, `src/parts/ui/widgets.ts`, режим бандла), и правка любого
...
/** Отпечаток каталога плагина строкой — тем же приёмом, что `fingerprintVersion` у виджета (`src/parts/ui/widgets.ts`), но по каталогу целиком. */
 ...
 * `src/parts/ui/daemon/kernel.ts`): откуда взят каталог плагина и каким файлом объявлена
 ...
 * (`src/parts/ui/daemon/kernel.ts`, действующий состав задачи 11), у которого есть дерево
 * и итоги его сборки.
 *
 * Это взгляд наблюдателя (`src/parts/ui/daemon/watcher.ts`): он отвечает на вопрос «на
 ...
 * (`src/parts/ui/daemon/server.ts`, `activePlugins`): членство — от демона, чтобы поток и
