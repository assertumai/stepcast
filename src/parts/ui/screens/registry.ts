...
import { isBuiltinRowFiber, rowScope, type BuiltinRow } from '../../../kernel/load.js';
import type { Kernel } from '../../../kernel/kernel.js';
import type { KernelCache } from '../pipelines.js';
import type { PluginsOverview } from '../plugins.js';
import type { Watcher } from '../daemon/watcher.js';
 ...
 * «куда послать запрос» (его читает диспетчер `src/parts/ui/daemon/server.ts`). Оба
 * заводит строка каркаса `ui-shell` (`../shell/row.ts`) вызовом `ctx.provide`, а не
 ...
 * `ContributionService.register` (`src/kernel/kernel.ts`): владелец
...
export interface RequestEnv {
   ...
   * то, что знает только `src/parts/ui/daemon/kernel.ts` (единая точка сборки), не сам
   ...
   * `../shell/row.ts`) спрашивает его на каждый такт наблюдателя, а не один раз на
   ...
   * `/plugins/<id>.js` (`src/parts/ui/daemon/server.ts`): строка, отключённая патчем или
   * отказавшая при применении, не уходит ни в поток, ни в ответ по адресу.
   *
   * Функция, а не значение: поток событий (`handleEvents`, `../shell/row.ts`) спрашивает
   ...
   * Решение 13) — поле окружения, а не прямой вызов `src/parts/ui/runLaunch.ts`, тем
...
export interface ActiveScreen {
  ...
  /**
   * Объявление внесла строка поставки витрины (`src/parts/ui/rows.ts`), а
...
/**
 * Маршруты глазами диспетчера (`src/parts/ui/daemon/server.ts`): только поиск, без
 * регистрации. Отдельный тип нужен `src/parts/ui/daemon/kernel.ts`: состав, собранный без
...
export class ApiService extends Service implements ApiRoutes {
   ...
   * 405 (адрес есть, но не этим методом) в диспетчере (`src/parts/ui/daemon/server.ts`).
   */
  hasAnyMethod(path: string): boolean {
