 ...
 * действующее ядро демона (`src/parts/ui/daemon/kernel.ts`). Здесь остаются: проверка
...
export interface UiServerOptions {
   ...
   * (ключ `home:<домашний каталог>`) — им пользуется `src/parts/ui/daemon/kernel.ts`.
   ...
   * процесс (`src/parts/ui/runLaunch.ts`); подмена нужна проверкам, которым нельзя
   ...
   * отсоединённый процесс `stepcast decide` (`src/parts/ui/runLaunch.ts`); подмена
 ...
 * (`daemon.plugins`, `src/parts/ui/daemon/kernel.ts` — патч и коллизии имени уже учтены),
...
async function handlePluginModule(
  ...
  // `src/kernel/tree/manifest.ts`) — демон отдаёт то, что назвал манифест, и
  // ничего сверх.
...
export function createUiServer(options: UiServerOptions): Promise<UiServer> {
  ...
  // же умолчанием, что и `currentDaemonKernel` (`src/parts/ui/daemon/kernel.ts`), — здесь
  ...
  // `src/parts/ui/daemon/kernel.ts`) живут в одном кеше — их пространства имён не
