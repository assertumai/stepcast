 ...
 * диспетчер адреса у демона (`src/parts/ui/daemon/server.ts`), записи прокси дев-сервера
 * (`vite.config.ts`) и перечень `external` при сборке половины
 * (`src/parts/ui/widgets.ts`). Имя входит в таблицу тогда и только тогда, когда два
...
/**
 * Имя экспорта модуля ошибки компиляции (`errorModuleText`, `src/parts/ui/widgets.ts`)
...
/**
 * Имя экспорта стилей собранной браузерной половины плагина (`src/parts/ui/widgets.ts`,
 ...
 * `isSafeSegment`, что и прочие пути витрины (`src/parts/ui/routes.ts`).
 */
 ...
 * `src/parts/ui/routes.ts`) — тем же предикатом, каким сервер проверяет сегменты
