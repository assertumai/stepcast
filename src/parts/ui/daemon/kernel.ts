...
import { resolveConfig, type ResolvedConfig } from '../../pipeline/config/resolve.js';
import type { RowOutcome } from '../../../kernel/load.js';
import { loadPlugins } from '../../load.js';
import { kernelFromRegistry, type Registry } from '../../../kernel/registry.js';
 ...
 * `ui-shell` и по строке на экран (`src/parts/ui/rows.ts`). Ею пользуются и
 * диспетчер маршрутов (`src/parts/ui/daemon/server.ts`), и `readSettings`, и `readModels`:
...
export interface DaemonKernel {
   ...
   * (`src/kernel/load.ts`) — колбэка, который зовётся ПО ХОДУ сборки
  ...
  /** Маршруты того же ядра — их ищет диспетчер (`src/parts/ui/daemon/server.ts`). */
   ...
   * умолчания за настройки пользователя (`src/parts/ui/settings.ts`,
   * `src/parts/ui/models.ts`).
   */
 ...
 * демонов в одном процессе (`src/parts/ui/pipelines.ts`, комментарий о кеше ядер),
