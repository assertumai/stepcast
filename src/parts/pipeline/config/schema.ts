...
/**
 * Имя записи таблицы раннеров: тот же образец, каким проверяется имя
 * MCP-сервера (`src/parts/pipeline/document/schema.ts`) — движок строит из него
 * составные идентификаторы (индекс расширений, отчёт `stepcast config`), и
 * пробел или точка там неуместны.
 */
const RunnerNameSchema = z
  .string()
  .regex(/^[A-Za-z0-9_-]+$/, 'имя раннера должно состоять из букв, цифр, дефиса и подчёркивания');

/**
 * Запись таблицы раннеров: «имя → argv-префикс» (design.md, решение 4).
 * `command` только списком argv — раннер запускается без оболочки, и правил
 * разбиения строки на слова у движка нет. `extensions` необязателен: запись,
 * добавленная только ради своего `runner:` на шаге, расширений не закрепляет.
 *
 * Необязателен и `command` — но только в отдельном слое: слои сливаются по
 * листьям в обе стороны, и требовать команду в каждом файле значило бы
 * запретить домашнему конфигу закрепить расширение за встроенным раннером,
 * не повторив его команду копией. Что запись, слитая из всех слоёв, команду
 * всё-таки имеет, проверяет `buildRunners` — там, где видны все слои сразу.
 */
export const RawRunnerSchema = z
  .object({
 ...
 *
 * Модель одна на конфигурацию и на документ пайплайна (`src/parts/pipeline/document/
 ...
 * по каким правилам они пишутся, каким инструментом заводятся и какой
 * командой проверяются. Каждый ключ необязателен сам по себе — слои
 * конфигурации сливаются по листьям, и репозиторий может объявить часть
 * группы. Умолчаний нет: путь, имя или команда, подставленные за автора,
 * указывали бы в чужом репозитории на несуществующее.
 */
export const RawSpecSchema = z
  .object({
    ...
    // Команда, которой движок проверяет документы изменения, — целиком, как
    // `project.check`, а не имя практики: движок её не разбирает и передаёт
    // оболочке как есть. Слаг изменения приходит переменной `SPEC_CHANGE`
    // (её объявляет работа петли — declare здесь ничего о ней не обязан
    // знать). Ключ не заменяет `tool`: `tool` — только имя для записи права
    // (`Bash(${params.spec_tool} *)`), а команду для исполнения собрать из
    // одного имени нельзя без знания грамматики подкоманд конкретной
    // практики.
...
export const RawProjectSchema = z
  .object({
    ...
    // (`ProjectSchema` в src/parts/pipeline/document/schema.ts его не заводит) и он
...
export const RawConfigSchema = z
  .object({
    ...
    // Таблица раннеров допустима в обоих слоях (в отличие от
    // `backends.*.command`): раннер обычно имя в PATH, и зависимость проекта
    // от него — законное содержимое проектного конфига (design.md, решение 6).
    runners: z.record(RunnerNameSchema, RawRunnerSchema).optional(),
