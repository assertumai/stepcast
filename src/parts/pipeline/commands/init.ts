...
import { StepcastError, ExitCode, type ExitCodeValue } from '../../../kernel/errors.js';
...
- \`scope\` — пути, при правке которых единица понадобится. По ним её находит
  отбор.
- \`anchors\` — пути, по которым обнаружится её устаревание. Закрепление —
  дайджест содержимого пути — подставляет движок: объявлять себя вечно
  свежим пишущий не вправе.
...
Записи единиц знания в оглавлении ограничены \`project.knowledge.index_max_tokens\`
— место в нём вынуждает единицы сливать вместо разрастания. Тело самой
единицы ограничено отдельно, \`project.knowledge.unit_max_tokens\`: за него
платит каждый отбор по области. Прежде чем заводить новую единицу, посмотрите,
нет ли уже утверждающей то же: обновить существующую лучше, чем завести
вторую.
`;
...
Предел \`index_max_tokens\` кладётся на записи единиц знания в оглавлении: он и
есть механизм против разрастания памяти — новая единица обязана отвоевать
строку в конечном оглавлении. Объём самого знания предел не обходит: тело
единицы ограничено отдельным ключом, \`unit_max_tokens\`.
