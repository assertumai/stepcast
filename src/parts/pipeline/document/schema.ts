 ...
 * (`src/parts/pipeline/expect/row.ts`). Умолчание четвёртого параметра
 ...
 * которые читает `src/parts/pipeline/document/steps.ts`: `params` обязана быть
 ...
 * пайплайна, который его использует. Эту функцию подаёт конструктору сервиса
 * `steps` строка `pipeline` (`src/parts/pipeline/row.ts`), заводящая сам
 * сервис, — параметром сборки ядра она не приходит вовсе
 * (`pipeline-owns-services`, Решение 1). Вклад и уже занятые вклады приходят в
 * неё непрозрачными значениями, так что доменного типа не узнаёт ни ядро, ни
 * механизм сервиса (`kernel-domain-free-imports`).
 */
...
export function buildDocumentSchemas(
   ...
   * безусловен перед прогоном (`src/parts/pipeline/commands/run.ts`), поэтому запись без
