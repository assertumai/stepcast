...
/**
 * Состояния пункта, смысл которых знает движок: открытое (`todo`), рабочее
 * (`in_progress`) и два терминальных. Отбор берёт в работу только первые два
 * (`select.ts`), `finish` проставляет только терминальные.
 *
 * Перечень — не граница формата. Статус пункта — любое слово той же формы
 * (`BacklogStatusSchema`): человек, ведущий очередь, вправе завести своё
 * состояние («отложено», «на ревью»), и из-за одного незнакомого слова
 * очередь не должна переставать разбираться целиком — вместе с отбором,
 * списком и доской. Движку незнакомый статус значит одно: пункт не свободен.
 *
 * Невидимым на доске такой пункт не становится: доска показывает его отдельно
 * и предлагает завести под его статус колонку (`screen-scrum`,
 * `.stepcast/board.yml`).
 */
export const BACKLOG_STATUSES = ['todo', 'in_progress', 'done', 'failed'] as const;

export type KnownBacklogStatus = (typeof BACKLOG_STATUSES)[number];

/** Форма статуса: строчное латинское слово, допускающее цифры и `_`, как `in_progress`. */
export const BACKLOG_STATUS_PATTERN = /^[a-z][a-z0-9_]*$/;

export const BacklogStatusSchema = z
  .string()
  .regex(BACKLOG_STATUS_PATTERN, 'должен быть словом из строчных латинских букв, цифр и _');
 ...
 * знает устройства проекта и не имеет права зависеть от `parts/pipeline/config`.
 */
 ...
 * (`stepcast project repos`, `src/parts/pipeline/domain/project/repos.ts`).
 */
