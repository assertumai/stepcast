...
export interface CleanTreeOptions {
   ...
   * составной якорь (`src/parts/pipeline/domain/anchor/composite.ts`). Читает его вызывающий
 ...
 * `checkWorkspaceAvailability` (`src/parts/pipeline/run/workspace.ts`), включая
 ...
 * `src/parts/pipeline/domain/anchor/composite.ts`: gitlink (путь равен самому объявленному
 ...
 * (`src/parts/pipeline/domain/lanes/merge.ts`).
 */
 ...
 * (`src/parts/pipeline/domain/anchor/composite.ts`).
 *
 ...
 * (`src/parts/pipeline/domain/lanes/merge.ts`). Ведёт ли корень часть — свойство самого
