...
import { StepcastError, isStepcastError } from '../../../../kernel/errors.js';
...
import { readManifest, readStatus } from '../../run/journal/reader.js';
import type { RunStatus } from '../../run/journal/schema.js';
import { atomicWrite } from '../../run/journal/writer.js';
import { readLock, type LockRead } from '../../document/lockRead.js';
 ...
 * (`src/parts/pipeline/commands/merge-lanes.ts`).
 */
...
export interface MergeLanesOptions {
   ...
   * каталогу — читает конфигурацию вызывающая команда
   * (`src/parts/pipeline/commands/merge-lanes.ts`), а не это ядро (design.md,
   * решение 7). Корневая команда
