...
// Реэкспорт: сам `findPackageRoot` переехал в ядро
// (`src/kernel/packageRoot.ts`, design.md, Решение 6), а публикуется — отсюда:
// подпуть `stepcast` (`parts/pipeline/index.ts`) берёт имя из этого модуля, и
// снятие реэкспорта убрало бы его из публичной поверхности, чего переезд не
// делает. Потребители внутри `parts/**` (`ui/routesFile.ts`, `ui/steps.ts`,
// `document/expand.ts`, `run/engine.ts`) по той же причине называют прежний
// адрес: их спецификаторы переезд не правит.
...
const PACKAGED_WRAPPERS: Readonly<Record<string, string>> = {
  step: join('dist', 'src', 'parts', 'pipeline', 'step', 'wrapper.js'),
};
